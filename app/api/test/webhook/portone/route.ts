...
export async function GET() { return json({ ok: true, method: "GET" }); }

export async function POST(req: Request) {
  const key = req.headers.get("x-internal-key") || "";
  if (key !== (process.env.INTERNAL_API_KEY ?? "")) return json({ ok: false, error: "unauthorized" }, 401);

  let body: any = {}; try { body = await req.json(); } catch {}
  const d: any = body?.data ?? body ?? {};

  const merchantUid: string | undefined = pick(d, "paymentId", "id", "merchantUid", "merchant_uid");
  ...
  // const transactionId: string | undefined = pick(d, "transactionId", "txId", "txid"); // ← 컬럼 없으면 쓰지 않음
  // const failure = BAD.has(statusRaw) ? (pick(d, "failure") ?? null) : null;          // ← 컬럼 없으면 쓰지 않음

  const mappedStatus = OK.has(statusRaw) ? "paid" : BAD.has(statusRaw) ? "failed" : (statusRaw || "unknown");
  ...
  // 2) payments 업데이트 — 존재 컬럼만!
  const upd: Record<string, any> = {
    status: mappedStatus,
    currency,
    updated_at: new Date().toISOString(),
  };
  if (typeof amount === "number") upd.amount = amount;
  else if (payRow.amount != null) upd.amount = payRow.amount;
  ...
  if (OK.has(statusRaw)) {
    ...
    if (user_id && plan_id) {
      const up1 = await supabaseAdmin
        .from("memberships")
        .update({ plan_id, status: "active", updated_at: new Date().toISOString() })
        .eq("user_id", user_id);
      if (up1.error) return json({ ok: false, step: "memberships.update", error: "update_failed", detail: up1.error }, 500);

      const chk = await supabaseAdmin.from("memberships").select("user_id").eq("user_id", user_id).maybeSingle();
      if (!chk.data) {
        const ins = await supabaseAdmin
          .from("memberships")
          .insert({ user_id, plan_id, status: "active", updated_at: new Date().toISOString() });
